...
if (isset($_FILES['csvFile']) && $_FILES['csvFile']['error'] === UPLOAD_ERR_OK) {
    ...
    foreach ($dataRows as $row) {
        ...
        $fields = [
            'ufCrm28City' => $city,
            'ufCrm28Community' => $community,
            'ufCrm28SubCommunity' => $sub_community,
            'ufCrm28Building' => $building,
            'ufCrm28Location' => $fullLocation,
        ];

        if (!isDuplicateBayoutLocation('city', $city)) {
        ...
        if (!isDuplicateBayoutLocation('community', $community)) {
            $response = CRest::call('crm.item.add', [
        ...
        if (!isDuplicateBayoutLocation('sub_community', $sub_community)) {
            $response = CRest::call('crm.item.add', [
        ...
        if (!isDuplicateBayoutLocation('building', $building)) {
            $response = CRest::call('crm.item.add', [
        ...
        if (!isDuplicateBayoutLocation('location', $fullLocation)) {
            $response = CRest::call('crm.item.add', [
    ...
    header('Location: ../index.php?page=bayut-locations');
    exit;
}
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    ...
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md relative">
        ...
        <!-- Form -->
        <form id="csvForm" action="./bayut.php" method="post" enctype="multipart/form-data">
            <div class="mb-4">
